...
class SMSC(object):
	...
	def _smsc_send_cmd(self, cmd, arg=""):
		url = ifs(os.environ.get("SMSC_HTTPS"), "https", "http") + "://smsc.ru/sys/" + cmd + ".php"
		arg = "login=" + quote(os.environ.get("SMSC_LOGIN")) + "&psw=" + quote(os.environ.get("SMSC_PASSWORD")) + "&fmt=1&charset=" + os.environ.get("SMSC_CHARSET") + "&" + arg
		...
		while ret == "" and i < 4:
			...
			try:
				if os.environ.get("SMSC_POST") or len(arg) > 2000:
					data = urlopen(url, arg.encode(os.environ.get("SMSC_CHARSET")))
				else:
